...
const { centroid } = require("@thi.ng/geom-poly-utils");
...
function setup(p5) {
  ...
  //   const pt = randomVectorInBoxWithMargin(0, 0, width, height, 100, 100);
  //   boxes.push(
  //     getRectShape(
  //       pt[0],
  //       pt[1],
  //       random(width / 13, width / 5),
  //       random(height / 13, height / 5),
  //       random(Math.TWO_PI)
  //     )
  //   );
  // }
  ...
  //   10
  // )((gx, gy, xSize, ySize) => {
  //   boxes.push(getRectShape(gx, gy, xSize, ySize, random(Math.TWO_PI)));
  // });
  let z = 0;
  sqGridPaddingCallback(
    100,
    ...
    boxes.push(
      getRectShape(
        ...
        10 + 10 * p5.noise((gx / width) * 1.5, (gy / width) * 1.5)
      )
    );
    z += 0.05;
  });
...
class Shape {
  constructor(customPath, angle = 0, spacing = 10) {
    ...
    if (this.path[0] === this.path[this.path.length - 1]) {
      this.centroid = centroid(this.path.slice(0, -1));
    } else {
      this.centroid = centroid(this.path);
    }
    this.angle = angle;
...
function getRectShape(x, y, width, height, angle, lineFillSpacing) {
  const path = [];
  path.push([x, y]);
